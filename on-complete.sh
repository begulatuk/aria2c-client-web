...
if [[ $2 -eq 1 ]]; then # single file
	rclone -v --local-no-check-updated --ignore-existing --exclude "{*.txt,*.exe,*.aria2,*.nfo}" --config="rclone.conf" copy "$3" "DRIVE:$RCLONE_DESTINATION" 2>&1	
elif [[ $2 -gt 1 ]]; then # multiple file
	rclone -v --local-no-check-updated --ignore-existing --exclude "{*.txt,*.exe,*.aria2,*.nfo}" --config="rclone.conf" copy "$topPath" "DRIVE:$RCLONE_DESTINATION/${relativePath%%/*}"
fi
